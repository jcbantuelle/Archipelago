...
def get_locations_by_region(world: Optional['LaMulanaWorld'], player: Optional[int], worldstate) -> Dict[str, List[LocationData]]:
	...
	locations = {
		...
		"Spring in the Sky [Main]": [
			...
			LocationData("Spring in the Sky - Ankh Jewel Chest", 2359123, lambda state: s.attack_chest(state) and (s.attack_earth_spear(state) or state.has_any({'Scalesphere', 'Sacred Orb'}, player)), is_cursable=True, file_type='rcd', zones=[4], room=7, screen=0, object_type=0x2c, item_id=19, obtain_flag=0x91, obtain_value=2),
