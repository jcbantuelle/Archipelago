...
def get_locations_by_region(world: Optional['LaMulanaWorld'], player: Optional[int], worldstate) -> Dict[str, List[LocationData]]:
	...
	locations = {
		...
		"Spring in the Sky [Main]": [
			...
			LocationData("Spring in the Sky - Ankh Jewel Chest", 2359123, lambda state: s.attack_chest(state) and (s.attack_earth_spear(state) or state.has_any({'Scalesphere', 'Sacred Orb'}, player)), is_cursable=True, file_type='rcd', zones=[4], room=7, screen=0, object_type=0x2c, item_id=19, obtain_flag=0x00, obtain_value=2),
		...
		"Dimensional Corridor [Upper]": [
			...
			LocationData("Dimensional Corridor - beolamu.exe Scan", 2359124, lambda state: state.has_all({'Angel Shield Children Defeated', 'torude.exe'}, player), file_type='rcd', zones=[17], room=8, screen=1, object_type=0xc3, item_id=95, obtain_flag=0x00, obtain_value=1),
